          ...
          <>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 max-w-4xl mx-auto">
              {upcomingProjects.map((project) => (
                <div
                  key={project.id}
                  className="glass-card rounded-2xl overflow-hidden group border border-outline-variant/10 relative hover:border-secondary/30 transition-all duration-300 flex flex-col justify-between"
                >
                  <div>
                    ...
                    <div className="aspect-video relative overflow-hidden bg-surface-container-high">
                      <img
                        alt={project.imageAlt}
                        className="w-full h-full object-cover blur-[1px] opacity-80 group-hover:scale-103 transition-transform duration-500"
                        src={project.image}
                      />
                      <div className="absolute inset-0 bg-gradient-to-t from-background/90 via-background/40 to-transparent"></div>
                      
                      {/* Pulsating Badge */}
                      <div className="absolute top-4 right-4 flex items-center gap-1.5 px-3 py-1 bg-surface-container-highest/90 text-secondary rounded-full text-[10px] font-bold uppercase tracking-wider backdrop-blur-sm border border-secondary/20">
                    ...
                    <div className="p-8">
                      ...
                      <div className="flex flex-wrap gap-2 mb-4">
                        {project.tags.map((tag, index) => (
                          <span
                            key={index}
                            className="px-2.5 py-1 rounded bg-surface-container-highest text-[10px] font-label text-on-surface-variant uppercase border border-outline-variant/10"
                          >
                            {tag}
                          </span>
                        ))}
                      ...
                      {/* Title and Description */}
                      <h4 className="font-headline text-xl font-bold mb-3 text-on-surface group-hover:text-secondary transition-colors">
                        {project.title}
                      </h4>
                      <p className="text-on-surface-variant text-sm mb-6 leading-relaxed">
                        {project.description}
                  ...
                  <div className="px-8 pb-8">
                    <div className="mb-6">
                      <div className="flex justify-between items-center mb-1.5">
                        <span className="text-xs font-semibold text-on-surface-variant">Development Progress</span>
                        <span className="text-xs font-bold text-secondary">{project.progress}%</span>
                      </div>
                      <div className="w-full bg-surface-container-highest rounded-full h-2 overflow-hidden border border-outline-variant/10">
                        <div
